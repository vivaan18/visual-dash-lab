...
import defaultTemplates from './templateGenerator';
import { AITemplateGenerator } from './AITemplateGenerator';
import ImageImporter from './ImageImporter';
import type { DashboardComponent } from '@/types/dashboard';

type Template = { id: string; name: string; description: string; thumbnail: string; components: DashboardComponent[] | any[] };

interface TemplateSelectorProps { onApplyTemplate: (template: any) => void; onClose: () => void; canvasWidth?: number; defaultColor?: string }

const TemplateSelector: React.FC<TemplateSelectorProps> = ({ onApplyTemplate, onClose, canvasWidth, defaultColor }) => {
  ...
  const handleTemplateGenerated = (template: any) => {
    const t: Template = { id: template.templateName ? template.templateName.replace(/\s+/g, '-').toLowerCase() : `ai-${Date.now()}`, name: template.templateName || template.name || 'AI Template', description: template.templateDescription || template.description || '', thumbnail: '✨', components: template.components || [] };
        ...
        <DialogContent className="max-w-7xl w-[90vw] h-[90vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
               ...
               <div className="flex gap-2">
                 <Button onClick={() => setShowAIGenerator(true)} className="gap-2" size="sm"><Sparkles className="h-4 w-4" /> Generate with AI</Button>
                 <Button variant="outline" onClick={() => setImportOpen(true)} className="gap-2" size="sm">Import From Image</Button>
               </div>
             </div>
          </DialogHeader>

          <ScrollArea className="flex-1 w-full">
            {customTemplates.length > 0 && (<div className="px-4 pt-2"><h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" />AI Generated Templates</h3></div>)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-4">
              {allTemplates.map((template) => (
                <Card key={template.id} className="cursor-pointer hover:shadow-lg transition-all duration-200 hover:scale-105 h-fit">
                  <CardHeader className="pb-2"><div className="text-2xl text-center mb-1">{template.thumbnail}</div><CardTitle className="text-xs text-center font-semibold leading-tight">{template.name}</CardTitle></CardHeader>
                  <CardContent className="pt-0"><p className="text-xs text-gray-600 dark:text-gray-400 mb-3 text-center leading-relaxed line-clamp-3">{template.description}</p><Button className="w-full text-xs" size="sm" onClick={() => { onApplyTemplate(template); onClose(); }}>Use Template</Button></CardContent>
                </Card>
              ))}
            </div>

            <div className="text-center p-4 text-sm text-gray-500">Showing all {allTemplates.length} templates</div>
          </ScrollArea>

          <div className="flex justify-center p-4 border-t flex-shrink-0"><Button variant="outline" onClick={onClose}>Cancel</Button></div>
        </DialogContent>
      </Dialog>

      <AITemplateGenerator open={showAIGenerator} onClose={() => setShowAIGenerator(false)} onTemplateGenerated={handleTemplateGenerated} canvasWidth={canvasWidth} defaultColor={defaultColor} />
      <ImageImporter open={importOpen} onClose={() => setImportOpen(false)} onTemplateGenerated={(p) => { console.log('image templates', p); /* hook in real handler */ }} />
    </>
  );
};

export default TemplateSelector;
export { AITemplateGenerator };
// import React, { useState } from 'react';
// import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
// import { Button } from "@/components/ui/button";
// import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
...
// // Generate the remaining 24 templates using the same pattern
...
// console.log('TemplateSelector module loaded with', templates.length, 'templates');
...
//  */
// const generateCompactGridLayout = (components: Omit<DashboardComponent, 'position' | 'zIndex'>[]): DashboardComponent[] => {
...
//     const REFERENCE_VIEW_WIDTH = 1300; // Reference canvas width for centering
...
// end of file - retained only the top TemplateSelector implementation and its export